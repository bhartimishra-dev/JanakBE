...
**It's now a permanent, stored file, generated once at order placement** — not rebuilt from scratch on every download. `checkout.service.ts` generates the PDF the moment `POST /checkout/place-order` succeeds, saves it to `public/uploads/invoices/<orderId>.pdf` (served statically, same convention as product images — `uploads:` is a persistent Docker volume in prod so it survives redeploys), and stores the absolute URL on `Order.invoiceUrl`. This endpoint just serves that stored file from then on: two downloads of the same order return byte-identical PDFs, even if the buyer's `CompanyProfile` or the seller's constants change afterward — it's a historical record, not a live snapshot. Generation is best-effort and never blocks the order itself: if it fails (disk full, etc.), the order still places successfully and this endpoint transparently falls back to generating (and backfilling) one on the next request. Orders placed before this existed have no stored file yet — the first `GET .../invoice` call for one of those generates it live and backfills it to disk, so every later call for that same order serves that same backfilled file too.

`Order.invoiceUrl` (and the equivalent field in `GET /admin/orders` list rows) is `null` until a stored copy exists — check it before assuming a direct link is available; if it's `null`, hitting this endpoint once will populate it.

**Customer-side equivalent**: `GET /orders/:id/invoice` (not under `/admin`) — same PDF, same stored-file-first + backfill behavior, but scoped to the caller's own orders. It reuses the existing `GET /orders/:id` ownership check, so requesting someone else's order id returns a plain `404 "Order not found"`, not a `403` (avoids confirming the order even exists to someone who doesn't own it). `invoiceUrl` is also present on the plain `GET /orders` / `GET /orders/:id` responses for a customer, same as admin's — a frontend can link directly to that URL instead of calling the download endpoint, since the static file itself isn't auth-gated (only the two API endpoints that generate/backfill it are); the URL is only as guessable as the order's UUID.
